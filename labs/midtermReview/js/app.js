...
function ballProperties() {
  ...
  if (ball.xPos + ball.width + ball.xSpeed > paddle.xPos && ball.yPos + ball.xSpeed < paddle.xPos + paddle.width && ball.yPos + ball.length > paddle.yPos && ball.yPos < paddle.yPos + paddle.length) {
    ball.xSpeed *= -1;
  }
  else if (ball.xPos > canvasWidth) {
      ball.xSpeed *= -1;
    }
    if (ball.xPos + ball.width > paddle.xPos && ball.xPos < paddle.xPos + paddle.width && ball.yPos + ball.length + ball.ySpeed > paddle.yPos && ball.yPos + ball.ySpeed < paddle.yPos + paddle.length) {
      ball.ySpeed *= -1;
    }
